...
/* Converted from IrisGL to OpenGL by Bill Sherman, October 2005 */
#include <stdio.h>
#include <stdlib.h>
#ifdef IGL_VERSION
#  include <gl.h>
#  include <device.h>
#else
#  include <GL/gl.h>					/* IGL: <gl.h> & <device.h> */
#endif
...
float	hstrlength(char *);	/* Length of a set of Hershey characters  */

float	hgetfontwidth(); /* The width of this font */
float	hgetfontheight(); /* The height of this font */
extern	char	*hallocate(unsigned);

extern void check_loaded(char *);
/*
 * hfont
...

{
...
#ifdef DEBUG
	printf("about to open font file '%s'\n", fontname);
#endif
...
#else
	if (fread(&nchars, 2, 1, fp) != 1)		/* The size of the data field doesn't change when changing architectures -- it's always two bytes */
#endif
		return (0);
	 ...
	 */
#ifdef IGL_VERSION /* I don't know where this "delete()" function comes from */
	if (hLoaded) {
		if (ftab.ind[0])
			delete (ftab.ind[0]);
		if (ftab.ind)
	...
	while(p < e) {
		...
		if (x != -50) {			/* means move */
			...
			if (Move) {
				Move = 0;
#ifdef IGL_VERSION
				rmv((Coord)xs, (Coord)ys, (Coord)0.0);	/* OGLXXX */
#else
				glTranslatef(xs, ys, 0.0);		/* IGL: rmv((Coord)xs, (Coord)ys, (Coord)0.0); */
#endif
			} else {
#ifdef IGL_VERSION
				rdr((Coord)xs, (Coord)ys, (Coord)0.0);	/* OGLXXX */
#else
#  ifdef DEBUG
				printf("printing a line to %f %f\n", xs, ys);
#  endif
				glLineWidth(5.0);
#  if 0
				glColor3ub(200,200,200); /* Temp: removed */
#  endif

				/* These four lines may actually be a no-op, since glEnd() pops back to last graphics position before glBegin() */
				glBegin(GL_LINES);
				glVertex2f(0.0, 0.0);
				glVertex2f(xs, ys);
				glEnd();

				glTranslatef(xs, ys, 0.0);		/* IGL: rdr((Coord)xs, (Coord)ys, (Coord)0.0); */
#endif
			}
...
#ifdef IGL_VERSION
	rmv((Coord)cx, (Coord)cy, (Coord)0.0);	/* OGLXXX */
#else
	glTranslatef(cx, cy, 0.0);					/* IGL: rmv((Coord)cx, (Coord)cy, (Coord)0.0); */
#endif
...

{
	...
	move2(x, y);	/* OGLXXX */
#else
	glTranslatef(x, y, 0);						/* IGL: move2(x, y); */
#endif
	hcharstr(s);
